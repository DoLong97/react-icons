...
function IconBase(props) {
    var elem = function (conf) {
        var computedSize = props && props.size || conf && conf.size || "1em";
        var className;
        if (conf && conf.className)
            className = conf.className;
        if (props && props.className)
            className = (className ? className + ' ' : '') + props.className;
        var attr = props && props.attr, title = props && props.title, svgProps = __rest(props, ["attr", "title"]);
        return (React.createElement("svg", __assign({ stroke: "currentColor", fill: "currentColor", strokeWidth: "0" }, conf && conf.attr, attr, svgProps, { className: className, style: __assign({ color: props && props.color || conf && conf.color }, conf && conf.style, props && props.style), height: computedSize, width: computedSize, xmlns: "http://www.w3.org/2000/svg" }),
